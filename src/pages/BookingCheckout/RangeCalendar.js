...
this.props.closeHourModal&&this.props.closeHourModal();
    ...
    shouldDisableDates=(date,min,max)=>{
  	...
  	if(this.props.blockedslots){
  		var selectedDate=dateFormat(date,'yyyy-mm-dd');
  		var filterRecords=this.props.blockedslots.filter((obj)=>obj.selected_date==selectedDate&&obj.blockedCheccking==true)
...
const {fromDate,endDate}=this.state;
	...
	// if(key=='dateValue'){
		if(fromDate && endDate){
			this.setState({endDiv:'',endDate:null,startdiv:'firstDivSelected',enddiv:'',fromDate:null},function(){
				// this.props.sendDate&&this.props.sendDate({fromDate:fromDate,endDate:endDate});
			})
		...
		if(fromDate){
			...
			this.props.submitDate&&this.props.submitDate({fromDate:tmp2,endDate:tmp1});
			});

				return;
			}
			this.setState({endDate:date,enddiv:'endDivSelected'},function(){
			this.props.submitDate&&this.props.submitDate({fromDate:fromDate,endDate:date});
  ...
  compnentWillMount(){
	...
	render() {
		...
		// console.log()
		return (
			<div className="HourlyAvailablityDiv rangecalendardiv BigCalendarMUI resetheader">
